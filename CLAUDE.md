...
- `tests/unit/` — QtTest executables registered with CTest, including `hatt-ui-shell-tests` (`MainWindowTests.cpp`), `hatt-design-canvas-tests` (`DesignCanvasTests.cpp`), `hatt-project-file-tests`, `hatt-component-library-tests` and `hatt-project-safety-tests` (`ProjectSafetyTests.cpp`; drives dialogs with a polling `onNextModal` helper). New test files need to be added to `tests/unit/CMakeLists.txt`.
...
- **Tool model:** one exclusive `QActionGroup` (`toolActions_`) holds the eleven mode actions `hatteda.tool.select|component|package|connect|via|pad|zone|terminal|probe|draw|measure` (shortcuts V, A, K, W, I, O, Z, R, P, D, M; package/via/pad/zone are Kayra only, probe Mergen only; zone mode (ADR-0012) draws `CanvasTool::Zone` copper/keepout/area zones and lists the board's zones in `ZoneList`). The same `QAction`s back the rail buttons, so rail, menu and shortcut can never disagree. `activateToolMode` rebuilds `ObjectSelector` (component mode, Proteus style: schematic shows the project pick list `MainWindow::projectDevices()` = `projectDeviceList(ProjectLibrary, schematic)` with `DeviceBar` buttons `hatteda.devices.pick` (`PickDevicesDialog`: `DeviceSearch`, `DeviceResults`) / `hatteda.devices.remove`, persisted as `.hatt` `library.devices`; `hatteda.devices.new` opens `DeviceEditorDialog` and Design › New footprint `FootprintEditorDialog` (ADR-0007, `ComponentLibrary.hpp`/`LibraryDialogs.hpp`: project devices/footprints with `project.device.`/`project.footprint.` UUID ids, registered at runtime via `registerProjectLibrary` → `registerSymbols`, stored as `library.customDevices|customFootprints`; footprints are parametric `FootprintParams` or explicit `pads`/`pins`/`shapes`); the board shows `unplacedBoardParts` rows placed via `DesignCanvas::setPlacementTemplate`, refreshed on `documentChanged` by `refreshComponentList`. Terminal/probe modes list symbols of the mode's `SymbolCategory`; draw mode the draw tool list), and `applyObjectSelection` maps the mode + selected row to `DesignCanvas::setTool(CanvasTool, variant)`. The last row per (mode, workspace) is remembered. Probe mode is disabled in Kayra. Canvas Esc with nothing pending emits `selectToolRequested`, which returns to Select mode.
...
- `SketchModel.hpp` is the **interim** editor model (ADR-0002): `Workspace {Schematic, Board}`, `SketchItem` (`Kind`: Symbol, Wire, Line, Polyline, Rectangle, Circle, Arc, Text; `points` in floating-point mm, `variant`, `label`, `quarterTurns`, `closed`), `SketchDocument = QVector<SketchItem>` (selection is a list of indices into it). Board outline and zones are `Polyline` items with variant `board-outline` / `copper-zone` / `keepout-zone` / `area-zone` (`isZoneVariant`; zones carry `net` and `zoneFill` Solid/Hatched/Empty, ADR-0009/0012).
