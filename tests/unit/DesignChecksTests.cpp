...
private slots:
    ...
    // Issue #47: boardTrackNets resolves each track (Wire) item to its schematic net name, keyed
    // by item id — a named net (ground, "0") and an auto-generated one (the divider midpoint, no
    // terminal touches it so buildConnectivity names it "N<n>").
    void boardTrackNetsResolveNamedAndAutoGeneratedNets() {
        const SketchDocument schematic = dcDividerExample();
        SketchDocument board = transferToBoard(schematic, {outline(80, 60)}).document;
        const auto r1 = *std::find_if(board.begin(), board.end(), [](const SketchItem& i) { return i.label == QLatin1String("R1"); });
        const auto r2 = *std::find_if(board.begin(), board.end(), [](const SketchItem& i) { return i.label == QLatin1String("R2"); });
        const QPointF groundPad = itemPads(r2)[1].center;   // net "0"
        const QPointF midpointPad = itemPads(r1)[1].center; // divider midpoint: auto-generated net

        const SketchItem namedTrack = track({groundPad, groundPad + QPointF(0, 5)});
        const SketchItem autoTrack = track({midpointPad, midpointPad + QPointF(0, 5)});
        board.append(namedTrack);
        board.append(autoTrack);

        const QHash<QString, QString> nets = boardTrackNets(schematic, board);
        QCOMPARE(nets.value(namedTrack.id), QStringLiteral("0"));
        const QString autoNet = nets.value(autoTrack.id);
        QVERIFY(!autoNet.isEmpty());
        QVERIFY(autoNet != QStringLiteral("0"));
        QVERIFY2(autoNet.startsWith(QLatin1Char('N')), qPrintable(autoNet));

        // A schematic error means the nets are unknown, so no track gets a label.
        SketchDocument broken = schematic;
        broken[0].label.clear(); // duplicate/missing reference -> erc.analysis error
        QVERIFY(boardTrackNets(broken, board).isEmpty());
    }
